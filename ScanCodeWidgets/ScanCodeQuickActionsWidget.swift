...
struct ScanCodeWidgetEntry: TimelineEntry {
    let date: Date
    let actions: [ScanAppAction]
}

struct ScanCodeWidgetProvider: TimelineProvider {

    func placeholder(in context: Context) -> ScanCodeWidgetEntry {
        ScanCodeWidgetEntry(date: .now, actions: ScanAppAction.allCases)
    }

    func getSnapshot(in context: Context, completion: @escaping (ScanCodeWidgetEntry) -> Void) {
        completion(placeholder(in: context))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<ScanCodeWidgetEntry>) -> Void) {
        let entry = ScanCodeWidgetEntry(date: .now, actions: ScanAppAction.allCases)
        completion(Timeline(entries: [entry], policy: .never))
    }
}

struct ScanCodeQuickActionsWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "com.codex.scancode.quick-actions", provider: ScanCodeWidgetProvider()) { entry in
            ScanCodeQuickActionsWidgetView(entry: entry)
        }
...
private struct ScanCodeQuickActionsWidgetView: View {
    @Environment(\.widgetFamily) private var family

    let entry: ScanCodeWidgetEntry
